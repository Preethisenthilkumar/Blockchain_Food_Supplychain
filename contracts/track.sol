...
contract Verifier{
    ...
    public constant returns(uint) {
        ...
        (entityDB[msg.sender].password == _password));
        uint val;
        if(entityDB[msg.sender].entity_type == 0){
            val = 0;
        }else if(entityDB[msg.sender].entity_type == 1){
            val = 1;
        }else if(entityDB[msg.sender].entity_type == 2){
            val = 2;
        }else if(entityDB[msg.sender].entity_type == 3){
            val = 3;
        }
        return val;
    }
